...
try{
    ...
    $ifQuerySetting="SENDMAILIF_QUERYRETURNSRESULTS=select * from query_store.staging_data(true) where user_id !=10 and mean_time >= 5000"
    $thenQueriesSetting="LIST_OF_QUERIESWITHSUPPORTINGDATA={""""queries_in_progress_for_more_than_5_minutes"""":""""select datname as Database, pid as Process_ID, usename as Username, query,client_hostname,state, now() - query_start as Query_Duration, now() - backend_start as Session_Duration from pg_stat_activity where age(clock_timestamp(),query_start) > interval '5 minutes' and state like 'active' and usename not like 'postgres' order by 1 desc;"""",""""list_of_processes_at_the_time_of_alert"""":""""select now()-query_start as Running_Since,pid,client_hostname,client_addr, usename, state, left(query,60) as query_text from pg_stat_activity;""""}"
    #++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
